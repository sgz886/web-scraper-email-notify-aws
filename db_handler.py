...
class DynamoDBHandler:
    ...
    def create_table_if_not_exists(self):
        ...
        try:
            ...
            logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Table {TABLE_NAME} already exists")
            return
            ...
            logger.info(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Table {TABLE_NAME} created successfully")
    
    def save_scan_result(self, files):
    ...
    def get_last_scan_result(self):
        ...
        try:
            ...
            logger.error(f"Error getting last scan result: {str(e)}")
            return [] 