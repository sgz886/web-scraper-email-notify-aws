...
class Config:
    ...
    def _load_config(self):
        ...
        secret = self._get_secret('xiaomi_eu_crawler_secrets')
        if secret:
            self._load_from_secrets(secret)
        else:
            self._load_from_env()

    def _load_from_secrets(self, secret):
        secrets_dict = json.loads(secret)
        self.AWS_ACCESS_KEY = secrets_dict.get('AWS_ACCESS_KEY')
        self.AWS_SECRET_KEY = secrets_dict.get('AWS_SECRET_KEY')
        self.SENDER_EMAIL = secrets_dict.get('SENDER_EMAIL')
        self.NEW_FILE_RECIPIENT_EMAILS = secrets_dict.get('NEW_FILE_RECIPIENT_EMAILS')
        self.LOG_RECIPIENT_EMAILS = secrets_dict.get('LOG_RECIPIENT_EMAILS')
